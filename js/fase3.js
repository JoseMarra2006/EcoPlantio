...
const MOUND_IMAGE_SRC = 'itens_jogo_img/nivel_3/planta_molhada.png'; // Imagem do montículo de terra
...
function createHoleGrid() {
    ...
    for (let i = 0; i < TOTAL_HOLES; i++) {
        ...
        holeButton.classList.add('plant-slot');
        holeButton.setAttribute('data-index', i);
        holeButton.innerHTML = `<img src="${HOLE_IMAGE_SRC}" alt="Planta para Adubar">`;
...
function handlePlanting(index, buttonElement) {
    ...
    if (!selectedSeedType) {
        alert('Selecione um agrotóxico (bom ou ruim) primeiro!');
        return;
    }
...
function handleSeedSelection() {
    const seedButtons = document.querySelectorAll('.plant-button');
...
Plantas Adubadas: ${planted}
Agrotóxicos Usados: ${badSeeds}
